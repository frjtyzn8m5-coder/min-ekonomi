...
export default function Overview() {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-4 lg:space-y-6">
      <motion.div {...FADE} transition={{ duration: 0.3 }} className="flex items-center justify-between">
        <div>
      ...
      <motion.div {...FADE} transition={{ duration: 0.3, delay: 0.05 }}
        ...
        {[
          ...
          },
          {
            ...
            value: lastMonth ? formatSEK(lastMonth.expenses) : '—',
            color: 'text-gray-900',
            sub: expPct !== 0 && prevMonth
      ...
      {savingsRate !== null && (
        <motion.div {...FADE} transition={{ duration: 0.3, delay: 0.08 }}>
          <div className={`rounded-xl px-5 py-3 flex items-center justify-between ${
            savingsRate >= 20 ? 'bg-green-50 border border-green-100' :
            savingsRate >= 0  ? 'bg-yellow-50 border border-yellow-100' :
                                'bg-red-50 border border-red-100'
          }`}>
            <div>
              <p className="text-xs text-gray-500">Sparkvot {lastMonth ? formatMonth(lastMonth.month) : ''}</p>
              <p className={`text-2xl font-bold tracking-tight ${
                savingsRate >= 20 ? 'text-green-700' :
                savingsRate >= 0  ? 'text-yellow-700' :
                                    'text-red-600'
              }`}>{savingsRate}%</p>
            </div>
            <p className="text-xs text-gray-400 text-right max-w-[180px] hidden sm:block">
              {savingsRate >= 20 ? 'Bra sparande! Håll i det.' :
               savingsRate >= 10 ? 'Okej — kan förbättras.' :
               savingsRate >= 0  ? 'Låg sparkvot den här månaden.' :
                                   'Utgifter översteg inkomsten.'}
            </p>
          </div>
        </motion.div>
      )}
          ...
          <Card>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={monthlyData.map(d => ({
                name: formatMonth(d.month), Inkomst: d.income, Utgifter: d.expenses, Sparande: d.savings,
              }))} barGap={2}>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={pieData} cx="50%" cy="50%" innerRadius={55} outerRadius={80} paddingAngle={3} dataKey="value">
                  {pieData.map(({ name }) => <Cell key={name} fill={CATEGORY_COLORS[name] || '#e5e7eb'} />)}
                </Pie>
      ...
      <motion.div {...FADE} transition={{ duration: 0.3, delay: 0.2 }}>
        <Card>
          ...
          <ResponsiveContainer width="100%" height={160}>
            <AreaChart data={monthlyData.map(d => ({ name: formatMonth(d.month), Kassaflöde: d.cashflow }))}>
              <defs>
              ...
              <div key={tx.id} className="flex items-center justify-between px-5 py-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-gray-800 truncate max-w-[180px] lg:max-w-64">{tx.description}</p>
                    <p className="text-xs text-gray-400">{tx.date} · {tx.category}</p>
                  </div>
                </div>
                <span className={`text-sm font-semibold flex-shrink-0 ml-3 ${tx.amount >= 0 ? 'text-green-600' : 'text-gray-800'}`}>
