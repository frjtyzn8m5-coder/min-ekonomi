            ...
            <div className="mb-4 space-y-2">
              <div className="grid grid-cols-2 gap-2">
                {(['A', 'B'] as const).map(slot => {
                  ...
                  return (
                    <div key={slot}>
                      <p className="text-[10px] text-gray-400 mb-1">Foto {slot}</p>
                      <select
              ...
              {compareA && compareB && (
                <div className="grid grid-cols-2 gap-2">
                  {[compareA, compareB].map(date => {
                    const entry = photoEntries.find(e => e.date === date);
                    return (
                      <div key={date} className="relative">
                        <img src={entry?.photoUrl} alt={date} className="w-full aspect-[3/4] object-cover rounded-xl" />
